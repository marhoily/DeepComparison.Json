...
namespace Tests
{
    public sealed class JsonFacts
    {
        ...
        [Fact]
        public void Property_String_Type_Does_Not_Match()
        {
            _comparer.Compare(
                    new JObject(new JProperty("Age", "blah")),
                    new { Age = 18})
                .Message.Should().Be("$root.Age: blah != 18");
        }
        [Fact]
        public void Compare_With_Null()
        {
            _comparer.Compare(
                    new JObject(new JProperty("Age", null)),
                    new { Age = 18})
                .Message.Should().Be("$root.Age: <null> != 18");
        }
        [Fact]
        public void Compare_With_Null_()
        {
            _comparer.Compare(
                    new JObject(new JProperty("Age", 18)),
                    new { Age = (string)null})
                .Message.Should().Be("$root.Age: 18 != <null>");
        }
        [Fact]
        public void Property_Numeric_Type_Does_Not_Match()
        {
            _comparer.Compare(
                    new JObject(new JProperty("Age", 27)),
                    new { Age = 27m })
